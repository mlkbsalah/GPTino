...
class AttentionHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        wght = q @ k.transpose(-2,-1) * k.shape[-1] ** -0.5 # (B, T, H) @ (B, H, T) -> (B, T, T)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, embed_size, context_window, dropout=0.2):
        super().__init__()
        self.heads = nn.ModuleList([AttentionHead(embed_size // n_heads, embed_size, context_window) for _ in range(n_heads)])
...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size, n_blocks, n_heads, embed_size, context_window):
        ...
        self.blocks = nn.Sequential(*[Block(n_heads, embed_size, context_window) for _ in range(n_blocks)])
